# MASTER_PLAN_v14 — UFOBeep
**User System Implementation & Premium Features + Multi-Media Alerts**
...
## MASTER PLAN v14 - MULTI-MEDIA ALERTS + UPDATE BEEPS

**SCOPE**
- Append only; do not overwrite earlier docs
- Keep /alerts as canonical noun
- No /v1. Version via header/config
- Secrets: /home/ufobeep/.secrets/

### A) MEDIA UPLOADS (PHOTOS + VIDEOS)

#### ENDPOINTS

**POST /media/uploads**
- multipart/form-data, repeated field `files`
- Allowed images: jpeg, png, webp, heic, heif (max 12MB each)
- Allowed videos: mp4, mov, avi, mpeg, webm (max 50MB each)
- Max files: 12 per upload (guardrail)
- Returns: `{ ok:true, media:[{url, type:"image"|"video"}] }`

**POST /alerts**
- Existing route; payload adds `media:[{url, type}]`
- Server stores all. The first media item becomes the **push preview**
- Push includes "+N more" if more media attached
- Feed card shows first media thumbnail + indicator of more

**POST /alerts/{id}/media**
- Auth required. Add media to an existing alert
- Body: `{ media:[{url, type}] }`
- Permission: Owner OR eligible witness (<=2× visibility + presence proof)
- On success: append to alert.media, regenerate gallery/thumbs, send update Beep (per user preference)
- Returns `{ ok:true, appended:N }`

#### STORAGE / PROCESSING
- Images: save + thumb + (later) blurred thumb
- Videos: save + ffmpeg poster frame thumb + (later) blurred poster
- DB fields:
  - `media:[{url,type,added_by,at}]`
  - `thumb_urls:[{url,type}]`
  - `blurred_media_urls:[]` (hooks only until moderation enforce)
  - `visibility:"visible"` (until enforcement flips)

### B) ALERT DISPLAY (APP + FEED)

#### FEED
- Show first media (thumb or poster)
- If more, overlay "+3 more"
- Push text: "New sighting with photo (+3 more)."

#### DETAIL SCREEN
- Gallery grid (images + video posters)
- Tap videos to play
- "Add media" button if:
  - user is owner OR can_add_media==true (server-computed)

### C) UPDATE BEEPS (USER PREFERENCE)

#### DEFAULT
- Adding media later (owner/witness) triggers **update Beep**
- Push text: "Update on sighting near {location}: new photos/videos added."

#### USER SETTING
- Each user can toggle: "Send me update Beeps when new media is added."
- Default: ON
- Stored in user profile in Firebase/Auth metadata
- If OFF: user still sees new media in gallery but no push

### D) MUFON INGEST

- Nightly job pulls MUFON sightings
- Same pipeline: multiple photos/videos → /media/uploads internal saver
- Fields: text_short, text_long, lat, lng, event_time, media[]
- Source="mufon"; created_by="system"; push=false
- Display:
  - Map: pin → short description
  - Detail: long description + gallery
  - Feed: chronological with "MUFON" badge

### E) MODERATION

- Hooks ON now: queue_moderate_image/videoPoster(url), store scores
- blurred_media_urls prepared
- Enforcement flag in /home/ufobeep/.secrets/app.env:
  ```
  MODERATION_ENFORCE=false  # default
  ```
- When MODERATION_ENFORCE=true:
  - explicit>=0.90 → block media (if all blocked, block alert, no push)
  - 0.50..0.89 → blur media, push text-only
  - Community flag endpoint: POST /alerts/{id}/flag-nsfw → immediate blur + review queue

### F) DOCS (APPEND)

#### endpoints.md:
- POST /media/uploads (supports image+video)
- POST /alerts (media array; first media = preview, push includes "+N more")
- POST /alerts/{id}/media (append media; owner/witness; triggers update Beep per user setting)
- GET /alerts/{id} (returns can_add_media; includes source/text_short/text_long for MUFON)

#### media_processing.md:
- Storage, thumbs, video posters (ffmpeg)
- NSFW hooks for images+posters
- MODERATION_ENFORCE flag behavior

#### TOC:
- Multi-media alerts (photos+videos)
- Feed push = first media + indicator
- Update Beeps (user preference; default ON)
- MUFON nightly ingest
- Moderation hooks/enforcement toggle

### G) ORDERING

1. Google login
2. Magic link login (fix flows, remove guest, add /auth/complete)
3. Owner multi-media upload (/media/uploads + /alerts)
4. Witness add-media (/alerts/{id}/media + eligibility)
5. Thumbs + video poster pipeline
6. Moderation hooks ON (MODERATION_ENFORCE=false)
7. MUFON nightly ingest
8. User settings screen: toggle update Beeps
9. Flip MODERATION_ENFORCE=true
10. After stable: rebrand to ufobeep.ai (DNS/TLS/Firebase/App links/redirects)
