...
---
### C3 — All External Feeds (API + Ops)
- Add NUFORC recent feed (HTML parser fallback).
- Consolidate ingest runner: `/admin/feeds/all/run` with per-source counts.
- Systemd timer `ufobeep-feeds.timer` @03:10 daily.
- Map filter chip: All | Community | MUFON | NUFORC
**Acceptance:** nightly inserts from both sources; dedupe holds; counts logged.

### DND/Quiet Hours — Guardrail Implementation (Feature-Flagged)
- Add `dnd_server_filtering` feature flag (default OFF).
- New V2 DTOs (`DeviceRegistrationRequestV2`, `PushTargetV2`) additive only.
- Add-only migration: quiet window fields (nullable).
- Dual-path: retain client muting; add server filter under flag.
- Sentry breadcrumbs + metrics for filter decisions.
- Canary -> staged rollout plan.
**Acceptance:** All unit/integration/E2E tests pass; 48h canary without new UI/DB errors; flips possible via flag.
