...
from ..feeds.ingest_alert_events import ingest_mufon as _ingest_mufon
from ..feeds.ingest_alert_events import ingest_all_feeds as _ingest_all
...
async def run_mufon(pool: asyncpg.Pool = Depends(get_db_pool), _admin = Depends(require_admin)):
    """Run MUFON authenticated feed ingestion into alert_events"""
    try:
        inserted = await _ingest_mufon(pool)
        return {"ok": True, "inserted": inserted, "source": "mufon_auth"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/all/run")
async def run_all(pool: asyncpg.Pool = Depends(get_db_pool), _admin = Depends(require_admin)):
    """Run all feeds ingestion (MUFON authenticated + NUFORC) into alert_events"""
    try:
